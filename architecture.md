...
| Routing | **Single route (`/`) with in-page anchors** | Matches the single-page IA in [scope.md](scope.md). `next/link` scroll-to-anchor covers navigation; no multi-page routing needed for v1 (no case-study sub-pages, no blog). |
...
| Deployment | **Vercel** | Native fit for Next.js App Router (ISR/SSG, image optimization, edge network), zero-config from a git push. |
